...
export class DeliveryNotesService {
  ...
  private async validatedLines(tx: Prisma.TransactionClient, context: TenantContext, input: any,
    ...
    const processedLines: Prisma.LineItemUncheckedCreateWithoutDeliveryNoteInput[] = normalized.map(l => {
      ...
      const totalPrice = new Prisma.Decimal(l.quantity).times(l.unitPrice)
        .times(new Prisma.Decimal(1).minus(new Prisma.Decimal(l.discountPercent || 0).div(100)))
  ...
  async update(context: TenantContext, id: string, dto: UpdateDeliveryNoteDto, key?: string) {
    ...
    return mutation(this.prisma, context, 'nexus:delivery-notes:update', 'update:' + id, key, dto, async tx => {
      ...
      const updateData: Prisma.DeliveryNoteUncheckedUpdateInput = {
        ...details,
        ...(lineItems ? { lineItems: { create: lineItems } } : {}),
      };
      const note = await tx.deliveryNote.update({ where: { id }, data: updateData, include: { lineItems: true } });
